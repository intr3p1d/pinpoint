...
public class ExceptionTraceServiceImpl implements ExceptionTraceService {
    ...
    @Override
    public List<ExceptionMetaData> getExceptionsInRange(
            ExceptionTraceQueryParameter queryParameter
    ) {
        return applyQueryFunction(
                queryParameter,
                this::getSimpleExeptionMetaDataList
        );
    }
    ...
    private List<ExceptionMetaData> getExeptionMetaDataList(ExceptionTraceQueryParameter queryParameter) {
        List<ExceptionMetaData> exceptionMetaData = exceptionTraceDao.getExceptions(queryParameter);
        logger.info(exceptionMetaData.size());
        return exceptionMetaData;
    }

    private List<ExceptionMetaData> getSimpleExeptionMetaDataList(ExceptionTraceQueryParameter queryParameter) {
        List<ExceptionMetaData> exceptionMetaData = exceptionTraceDao.getSimpleExceptions(queryParameter);
        logger.info(exceptionMetaData.size());
        return exceptionMetaData;
    }
    ...
    private List<ExceptionTraceValueView> getExceptionTraceValueViews(ExceptionTraceQueryParameter queryParameter) {
        List<ExceptionTraceValueView> exceptionTraceValueViews = exceptionTraceDao.getValueViews(queryParameter);
        logger.info(exceptionTraceValueViews.size());
        return exceptionTraceValueViews;
    }
}
